...
export function Calendar({
            ...
            tasks.filter((task) => {
                ...
                if (timeFilter !== "all") {
                    ...
                    const daysDiff = Math.ceil(
                        (task.startDate.getTime() - now.getTime()) /
                            (1000 * 60 * 60 * 24)
                    );
                ...
                if(!task.name.toLocaleLowerCase().startsWith(taskName.toLocaleLowerCase())) return false

                return true;
            }),
        [tasks, timeFilter, categoryFilters, userFilters, taskName]
    );
    ...
    //       weekStartDate.setDate(startOfToday.getDate() + weekStartOffset);

    //       const weekEndDate = new Date(startOfToday);
    //       weekEndDate.setDate(startOfToday.getDate() + weekEndOffset);

    //       return task.startDate >= weekStartDate && task.startDate < weekEndDate;
    //     }

    //     return true;
    //   }), [tasks, timeFilter, categoryFilters, userFilters]
    // );
    ...
    const getTasksForWeek = (weekStart: Date) => {
        ...
        return filteredTasks.filter((task) => {
            return weekDays.some(
                (day) => day >= task.startDate && day <= task.endDate
            );
        });
    };
    ...
    const isInResizePreview = (day: Date) => {
        ...
        if (resizingTask.mode === "right") {
            return (
                day >= resizingTask.task.startDate &&
                day <= (hoveredDay || resizingTask.task.endDate)
            );
        } else {
            return (
                day >= (hoveredDay || resizingTask.task.startDate) &&
                day <= resizingTask.task.endDate
            );
        }
    };
                    ...
                    <div className="grid grid-cols-7 gap-0.5 lg:gap-1 mb-2">
                        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
                            (day) => (
                                <div
                                    key={day}
                                    className="p-2 text-center font-medium text-muted-foreground text-xs lg:text-sm"
                                >
                        ...
                        <div key={weekIndex} className="relative mb-1">
                            ...
                            <div className="grid grid-cols-7 gap-0.5 lg:gap-1">
                                {week.map((day) => {
                                    ...
                                    const isDropTarget =
                                        draggedTask && isHovered;
                                    const isResizePreview =
                                        isInResizePreview(day);
                                    ...
                                    return (
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                (isSelected || (hoveredDay && isSameDay(day, hoveredDay))) &&
                                        ...
                                        >
                                            ...
                                            {isDropTarget && (
                                                <div className="absolute inset-0 bg-green-200/50 border-2 border-dashed border-green-400 rounded pointer-events-none flex items-center justify-center">
                                                    <span className="text-green-700 font-medium text-xs hidden sm:inline">
                                                        Drop here
                                                    </span>
                                                </div>
                                            )}
